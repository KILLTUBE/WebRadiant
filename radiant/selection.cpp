...
void testSelect( const View& view, const Matrix4& pivot2world ){
	...
	if( GlobalSelectionSystem().countSelected() != 0 ){
		...
		if( bestPointSelector.isSelected() ){
			...
			const Vector3 near_ = vector4_projected( matrix4_transformed_vector4( test.getScreen2world(), Vector4( 0, 0, -1, 1 ) ) );
			const Vector3 far_ = vector4_projected( matrix4_transformed_vector4( test.getScreen2world(), Vector4( 0, 0, 1, 1 ) ) );
			start = vector3_normalised( far_ - near_ ) * ( 256.f + GetGridSize() * sqrt( 3.0 ) ) + near_;
		}
